...
const SAMPLE = {
  ...
  capital:[], receipts:[],
  users:[{ id:"usr1", username:"admin", password:"admin123", role:"admin", name:"Ascentix Admin", email:"admin@ascentix.com.bd", llcId:null }],
  llcPayments:[], employees:[], payroll:[], bonusPool:[],
...
function calcLLCPL(llc, jobs, ins, usaExp, ym) {
  ...
  return { revenue,jobCosts,insInstallment,insAdvance,usaOps,mgmtFee:+mgmtFee.toFixed(2),
    ...
    ascentixEquityShare:+(operatingProfit*(llc.ar||0)/100).toFixed(2),
    ownerShare:+(operatingProfit*(llc.or||0)/100).toFixed(2),jobCount:myJobs.length };
}
...
function GroupsPage({data,onSave,showToast}){
  ...
  return(<div style={card}>
    ...
    {show&&(<AddBox onCancel={()=>setShow(false)} onSave={add} saveLabel="Create Group">
      <G2>
        <Field label="Group Name *"><input style={inp} value={form.name} onChange={e=>f("name",e.target.value)} /></Field>
...
function LLCsPage({data,onSave,showToast}){
  ...
  return(<div style={card}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
      <div><div style={{fontSize:"15px",fontWeight:600,color:C.text}}>LLC Entities</div><div style={{fontSize:"12px",color:C.textMuted}}>Use Edit to assign or change a portfolio group</div></div>
      <button style={btnP} onClick={()=>{cancelEdit();setShow(s=>!s);}}>+ Add LLC</button>
    </div>
    {show&&(<AddBox onCancel={cancelEdit} onSave={save} saveLabel={editId?"Update LLC":"Save LLC"}>
    ...
    {data.llcs.length===0?<Empty msg="No LLCs yet."/>:(
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
        ...
        <tbody>{data.llcs.map(l=>{const grp=data.groups.find(g=>g.id===l.groupId);return(
          <tr key={l.id}>
    ...
    {sorted.length===0?<Empty msg="No job entries yet."/>:sorted.map(j=>{
      ...
      return(<div key={j.id} style={{border:`1px solid ${open?C.indigoBorder:C.border}`,borderRadius:"8px",marginBottom:"10px",overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px",padding:"12px 14px",cursor:"pointer",background:open?C.indigoBg:C.card}} onClick={()=>setExpanded(open?null:j.id)}>
          ...
          <div style={{flex:"0 0 110px",fontSize:"12px",color:C.textMid,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{j.client||"—"}</div>
            ...
            return(<div key={v.id} style={{border:`1px solid ${C.border}`,borderRadius:"7px",marginBottom:"10px",background:C.card}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",borderBottom:v.payments.length>0||isAP?`1px solid ${C.border}`:"none"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                  <div style={{textAlign:"right"}}><div style={{fontSize:"11px",color:C.textMuted}}>Paid</div><div style={{fontWeight:600,color:vPaid>0?C.red:C.textMuted}}>{vPaid>0?fmt$(vPaid):"—"}</div></div>
              ...
              {isAP&&(<div style={{padding:"12px",background:C.indigoBg}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr auto",gap:"8px",alignItems:"flex-end"}}>
                  ...
                  <div style={{display:"flex",gap:"4px",paddingBottom:"1px"}}><button style={btnP} onClick={()=>addPayment(j.id,v.id)}>Save</button><button style={btnS} onClick={()=>setAddingPay(null)}>✕</button></div>
                </div>
              </div>)}
          ...
          {addingVendor===j.id?(
            <div style={{border:`1px solid ${C.indigoBorder}`,borderRadius:"7px",padding:"12px",background:C.card,marginBottom:"6px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:"8px",alignItems:"flex-end"}}>
                ...
                <div style={{display:"flex",gap:"4px",paddingBottom:"1px"}}><button style={btnP} onClick={()=>addVendor(j.id)}>Add</button><button style={btnS} onClick={()=>setAddingVendor(null)}>Cancel</button></div>
              </div>
            </div>
    ...
    {data.ins.length===0?<Empty msg="No policies."/>:data.ins.map(ins=>{
      ...
      return(<div key={ins.id} style={{border:`1px solid ${C.border}`,borderRadius:"8px",padding:"14px",marginBottom:"10px"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px"}}>
          <div><div style={{fontWeight:500}}>{llc?.name}</div><div style={{fontSize:"12px",color:C.textMid}}>{ins.provider} · {ins.policy} · {fmtDate(ins.start)}</div></div>
          <div style={{display:"flex",gap:"10px",alignItems:"center"}}><div style={{fontWeight:600}}>{fmt$(ins.total)}</div><button style={btnD} onClick={()=>del(ins.id)}>Delete</button></div>
        </div>
...
function USAExpPage({data,onSave,showToast}){
  ...
  return(<div style={card}>
    ...
    {show&&(<AddBox onCancel={()=>setShow(false)} onSave={add} saveLabel="Save">
      <G3>
        <Field label="LLC *"><select style={sel} value={form.llcId} onChange={e=>f("llcId",e.target.value)}><option value="">Select...</option>{data.llcs.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></Field>
        <Field label="Category"><select style={sel} value={form.cat} onChange={e=>f("cat",e.target.value)}>{["CPA","LLC Formation","Software","Other"].map(c=><option key={c} value={c}>{c}</option>)}</select></Field>
    ...
    {data.usaExp.length===0?<Empty msg="No USA expenses yet."/>:(
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
        ...
        <tbody>{[...data.usaExp].sort((a,b)=>b.date.localeCompare(a.date)).map(e=>{const llc=data.llcs.find(l=>l.id===e.llcId);return(<tr key={e.id}><td style={td}>{fmtDate(e.date)}</td><td style={td}>{llc?.name}</td><td style={td}><span style={badge("blue")}>{e.cat}</span></td><td style={{...td,color:C.red,fontWeight:500}}>{fmt$(e.amount)}</td><td style={{...td,color:C.textMid}}>{e.desc}</td><td style={td}><button style={btnD} onClick={()=>del(e.id)}>✕</button></td></tr>);})}</tbody>
      </table>
    )}
  </div>);}
...
function BDExpPage({data,onSave,showToast}){
  ...
  return(<div style={card}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
      <div><div style={{fontSize:"15px",fontWeight:600,color:C.text}}>Bangladesh Ops — Ascentix Entity</div><div style={{fontSize:"12px",color:C.textMuted}}>Total: {fmtBDT(total)} ≈ {fmt$(total/xRate)}</div></div>
      <button style={btnP} onClick={()=>setShow(!show)}>+ Add</button>
    </div>
    <div style={{height:"1px",background:C.border,margin:"12px 0"}} />
    {show&&(<AddBox onCancel={()=>setShow(false)} onSave={add} saveLabel="Save">
      <G3>
        <Field label="Category"><select style={sel} value={form.cat} onChange={e=>f("cat",e.target.value)}>{["Salary","Rent","Utility","Travel","Bonus","Website","Misc"].map(c=><option key={c} value={c}>{c}</option>)}</select></Field>
    ...
    {data.bdExp.length===0?<Empty msg="No BD expenses yet."/>:(
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
        <thead><tr>{["Date","Category","BDT","≈ USD","Description",""].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
        <tbody>
          {[...data.bdExp].sort((a,b)=>b.date.localeCompare(a.date)).map(e=><tr key={e.id}><td style={td}>{fmtDate(e.date)}</td><td style={td}><span style={badge("amber")}>{e.cat}</span></td><td style={{...td,fontWeight:500}}>{fmtBDT(e.amount)}</td><td style={{...td,color:C.red}}>{fmt$(e.amount/xRate)}</td><td style={{...td,color:C.textMid}}>{e.desc}</td><td style={td}><button style={btnD} onClick={()=>del(e.id)}>✕</button></td></tr>)}
...
function ExpensesPage(props){
  ...
  const tabs=[{id:"ins",label:"Insurance"},{id:"usa",label:"USA Operational"},{id:"bd",label:"Bangladesh (Ascentix)"}];
  return(<div>
    ...
    <div style={card}>
      ...
      {show&&(<AddBox onCancel={()=>setShow(false)} onSave={add} saveLabel="Save">
        <G2>
          ...
          <Field label="Type"><select style={sel} value={form.type} onChange={e=>f("type",e.target.value)}><option value="investment">Investment</option><option value="withdrawal">Withdrawal</option></select></Field>
          <Field label="Investor Name *"><input style={inp} value={form.investor} onChange={e=>f("investor",e.target.value)} /></Field>
          <Field label="Date"><input style={inp} type="date" value={form.date} onChange={e=>f("date",e.target.value)} /></Field>
      ...
      {data.capital.length===0?<Empty msg="No capital entries yet."/>:(
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
          ...
          <tbody>{[...data.capital].sort((a,b)=>b.date.localeCompare(a.date)).map(c=>{const llc=c.llcId?data.llcs.find(l=>l.id===c.llcId):null;return(
...
function ClientReceiptsPage({data,onSave,showToast}){
  ...
  const [show,setShow]=useState(false);const[form,setForm]=useState(blankR);const[allocs,setAllocs]=useState({});const[filterLLC,setFilterLLC]=useState("");const[viewReceipt,setViewReceipt]=useState(null);
  ...
  const saveReceipt=()=>{
    ...
    if(allocList.length===0){showToast("Allocate to at least one invoice","error");return;}
  ...
  const delReceipt=id=>{onSave("receipts",data.receipts.filter(r=>r.id!==id));};
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:"12px",marginBottom:"16px"}}>
      {[{l:"Total Invoiced",v:fmt$(totalInvoiced),c:C.text},{l:"Total Received",v:fmt$(totalReceived),c:C.green},{l:"Outstanding",v:fmt$(Math.max(0,totalInvoiced-totalReceived)),c:C.red},{l:"Collection Rate",v:`${totalInvoiced>0?((totalReceived/totalInvoiced)*100).toFixed(1):0}%`,c:C.indigo}].map((s,i)=>(
        <div key={i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"10px",padding:"14px 16px"}}><div style={{fontSize:"12px",color:C.textMuted}}>{s.l}</div><div style={{fontSize:"20px",fontWeight:700,color:s.c,marginTop:"4px"}}>{s.v}</div></div>
      ))}
    </div>
    <div style={card}>
      ...
      {show&&(<div style={{background:C.indigoBg,border:`1px solid ${C.indigoBorder}`,borderRadius:"8px",padding:"16px",marginBottom:"16px"}}>
        <G2>
          ...
          <Field label="Client"><input style={inp} value={form.client} onChange={e=>fr("client",e.target.value)} /></Field>
          ...
          <Field label="Method"><select style={sel} value={form.method} onChange={e=>fr("method",e.target.value)}>{PAY_METHODS.map(m=><option key={m} value={m}>{m}</option>)}</select></Field>
        ...
        {form.llcId&&+form.totalAmount>0&&(
          <div style={{marginTop:"14px"}}>
            ...
            {eligibleJobs.length===0?<div style={{fontSize:"13px",color:C.textMuted}}>No invoices for this LLC.</div>:(
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
                <thead><tr>{["Invoice Ref","Client","Invoiced","Received","Allocate"].map(h=><th key={h} style={{...th,fontSize:"11px"}}>{h}</th>)}</tr></thead>
                <tbody>{eligibleJobs.map(j=>{const rcv=jobAmountReceived(j.id,data.receipts);const owed=Math.max(0,j.amount-rcv);const st=arStatus(j,data.receipts);return(
                  <tr key={j.id}><td style={{...td,fontFamily:"monospace",fontSize:"11px"}}>{j.iRef}</td><td style={td}>{j.client||"—"}</td><td style={{...td,color:C.green,fontWeight:500}}>{fmt$(j.amount)}</td><td style={{...td,color:C.blue}}>{rcv>0?fmt$(rcv):"—"}</td><td style={{...td,width:"120px"}}>{st==="paid"?<span style={{fontSize:"11px",color:C.textMuted}}>Paid</span>:<input style={{...inp,padding:"4px 8px",fontSize:"12px"}} type="number" placeholder="0.00" value={allocs[j.id]||""} onChange={e=>setAllocs(p=>({...p,[j.id]:e.target.value}))} />}</td></tr>
                );})}</tbody>
              </table>
            )}
          </div>
        )}
        <div style={{display:"flex",gap:"8px",marginTop:"14px"}}><button style={btnP} onClick={saveReceipt}>Save Receipt</button><button style={btnS} onClick={()=>{setShow(false);setAllocs({});}}>Cancel</button></div>
      </div>)}
      {filteredReceipts.length===0?<Empty msg="No receipts yet."/>:filteredReceipts.map(r=>{
        const llc=data.llcs.find(l=>l.id===r.llcId);const isOpen=viewReceipt===r.id;
        return(<div key={r.id} style={{border:`1px solid ${isOpen?C.indigoBorder:C.border}`,borderRadius:"8px",marginBottom:"10px",overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 14px",cursor:"pointer",background:isOpen?C.indigoBg:C.card}} onClick={()=>setViewReceipt(isOpen?null:r.id)}>
            ...
            <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:"12px"}}>
              ...
              <div style={{display:"flex",gap:"4px"}}>
                <button style={{...btnS,padding:"4px 8px"}} onClick={e=>{e.stopPropagation();setViewReceipt(isOpen?null:r.id);}}>{isOpen?"▲":"▼"}</button>
          ...
          {isOpen&&(<div style={{borderTop:`1px solid ${C.indigoBorder}`,background:"#f8f9ff",padding:"14px 16px"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
              <thead><tr>{["Invoice Ref","Client","Invoiced","Allocated","Balance"].map(h=><th key={h} style={{...th,fontSize:"11px",background:"#eef2ff"}}>{h}</th>)}</tr></thead>
              <tbody>{r.allocations.map((a,ai)=>{const job=data.jobs.find(j=>j.id===a.jobId);if(!job)return null;const totalRcv=jobAmountReceived(job.id,data.receipts);const bal=job.amount-totalRcv;return(
                <tr key={ai}><td style={{...td,fontFamily:"monospace",fontSize:"11px"}}>{job.iRef}</td><td style={td}>{job.client||"—"}</td><td style={{...td,color:C.green,fontWeight:500}}>{fmt$(job.amount)}</td><td style={{...td,fontWeight:600,color:C.blue}}>{fmt$(a.amount)}</td><td style={td}><span style={badge(bal<=0?"green":bal<job.amount?"amber":"red")}>{bal<=0?"Paid":fmt$(bal)+" due"}</span></td></tr>
              );})}
              </tbody>
            </table>
          </div>)}
...
function LLCPaymentsPage({data,onSave,showToast,currentUser,viewLlcId}){
  ...
  const save=async()=>{
    ...
    onSave("llcPayments",[...data.llcPayments,{...form,id:uid(),llcId:isAdmin?myLlcId:currentUser.llcId,amount:+form.amount,recordedBy:currentUser.id,doc:null}]);
    setForm(blank);setShow(false);showToast("Payment recorded");
  };
    ...
    <div style={card}>
      ...
      {show&&!isAdmin&&(<div style={{background:C.indigoBg,border:`1px solid ${C.indigoBorder}`,borderRadius:"8px",padding:"16px",marginBottom:"16px"}}>
        <G2>
          ...
          <Field label="Date *"><input style={inp} type="date" value={form.date} onChange={e=>f("date",e.target.value)} /></Field>
          <Field label="Amount (USD) *"><input style={inp} type="number" value={form.amount} onChange={e=>f("amount",e.target.value)} /></Field>
          <Field label="Method"><select style={sel} value={form.method} onChange={e=>f("method",e.target.value)}>{PAY_METHODS.map(m=><option key={m} value={m}>{m}</option>)}</select></Field>
          <Field label="Reference"><input style={inp} value={form.ref} onChange={e=>f("ref",e.target.value)} /></Field>
        </G2>
      ...
      {filtered.length===0?<Empty msg="No payments recorded yet."/>:(
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
          ...
          <tbody>{filtered.map(p=>{const llc=data.llcs.find(l=>l.id===p.llcId);return(
            <tr key={p.id}>
              {isAdmin&&<td style={{...td,fontWeight:500,maxWidth:"130px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{llc?.name||"—"}</td>}
...
function ReportsPage({data,onSave}){
  ...
  const generate=()=>{
    ...
    const bdUSD=+(bdBDT/(data.settings.xRate||110)).toFixed(2);
  ...
  return(<div>
    ...
    {report&&(<div>
      <div style={{...card,borderLeft:`4px solid ${C.indigo}`}}>
        ...
        <div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>{data.llcs.length} LLC entities · {data.groups.length} portfolio groups</div>
      </div>
  ...
  return(<div>
    ...
    {show&&(<div style={{background:C.indigoBg,border:`1px solid ${C.indigoBorder}`,borderRadius:"8px",padding:"16px",marginBottom:"16px"}}>
      ...
      <G2>
        ...
        <Field label="Email"><input style={inp} value={form.email} onChange={e=>f("email",e.target.value)} /></Field>
        <Field label="Phone"><input style={inp} value={form.phone} onChange={e=>f("phone",e.target.value)} /></Field>
        <Field label="NID"><input style={inp} value={form.nid} onChange={e=>f("nid",e.target.value)} /></Field>
        <Field label="Bank Name"><input style={inp} value={form.bankName} onChange={e=>f("bankName",e.target.value)} /></Field>
        <Field label="Bank Account"><input style={inp} value={form.bankAccount} onChange={e=>f("bankAccount",e.target.value)} /></Field>
      </G2>
    ...
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
      ...
      <tbody>{data.employees.map(emp=>{const ps=calcPayslip(emp);return(
...
function MonthlyPayrollTab({data,onSave,showToast}){
  ...
  const generatePayroll=()=>{
    if(generated){showToast("Already generated","error");return;}
    ...
    onSave("payroll",[...data.payroll,...records]);showToast(`Payroll generated — ${records.length} employees`);
  };
  ...
  return(<div>
    ...
    {generated?(
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
        ...
        <tbody>{existing.map(p=>{const emp=data.employees.find(e=>e.id===p.empId);return(
          <tr key={p.id}>
            <td style={{...td,fontWeight:500}}><div>{emp?.name||"—"}</div><div style={{fontSize:"11px",color:C.textMuted}}>{emp?.designation}</div></td>
      ...
      <div>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
          ...
          <tbody>{previewRows.map(({emp,ps,netSalary})=>(
            <tr key={emp.id}>
...
function BonusTab({data,onSave,showToast}){
  const now=new Date();
  const [selMonth,setSelMonth]=useState(now.getMonth()+1);const[selYear,setSelYear]=useState(now.getFullYear());const[bForm,setBForm]=useState({llcId:"",poolPct:5,notes:""});const[distMap,setDistMap]=useState({});const[show,setShow]=useState(false);
  ...
  const deleteBP=id=>{onSave("bonusPool",data.bonusPool.filter(b=>b.id!==id));};
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
      <div><div style={{fontSize:"14px",fontWeight:600}}>Performance Bonus Pools</div></div>
    ...
    {show&&(<div style={{background:C.indigoBg,border:`1px solid ${C.indigoBorder}`,borderRadius:"8px",padding:"16px",marginBottom:"16px"}}>
      ...
      {bForm.llcId&&llcPL&&(
        <div style={{margin:"12px 0",padding:"10px 14px",background:C.card,borderRadius:"6px",display:"flex",gap:"20px",alignItems:"center"}}>
          ...
          <div><div style={{fontSize:"11px",color:C.textMuted}}>Pool USD</div><div style={{fontWeight:700,color:C.indigo}}>{fmt$(poolAmount)}</div></div>
      ...
      {bForm.llcId&&poolBDT>0&&(<div style={{marginTop:"12px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
          ...
          <div style={{display:"flex",gap:"6px"}}><button style={{...btnS,fontSize:"11px"}} onClick={autoDistribute}>Auto (by share %)</button><button style={{...btnS,fontSize:"11px"}} onClick={equalDistribute}>Equal split</button></div>
        </div>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px",marginBottom:"10px"}}>
          <thead><tr>{["Employee","Default %","Bonus (BDT)"].map(h=><th key={h} style={{...th,fontSize:"11px"}}>{h}</th>)}</tr></thead>
          <tbody>{eligibleEmps.map(emp=>(
            <tr key={emp.id}><td style={{...td,fontWeight:500}}>{emp.name}</td><td style={td}><span style={badge("purple")}>{emp.bonusSharePct}%</span></td><td style={{...td,padding:"6px 8px"}}><input style={{...inp,padding:"4px 8px",fontSize:"12px",width:"120px"}} type="number" placeholder="0" value={distMap[emp.id]||""} onChange={e=>setDistMap(p=>({...p,[emp.id]:e.target.value}))} /></td></tr>
          ))}</tbody>
        </table>
        <div style={{fontSize:"12px",color:totalDistributed>poolBDT?C.red:C.green,marginBottom:"8px"}}>{fmtBDT(totalDistributed)} of {fmtBDT(poolBDT)} distributed</div>
    ...
    {[...data.bonusPool].sort((a,b)=>(b.createdAt||"").localeCompare(a.createdAt||"")).map(bp=>{
      const llc=data.llcs.find(l=>l.id===bp.llcId);const[py,pm]=(bp.period||"-").split("-").map(Number);const paidCount=bp.distributions.filter(d=>d.status==="paid").length;
      return(<div key={bp.id} style={{border:`1px solid ${C.border}`,borderRadius:"8px",marginBottom:"12px",overflow:"hidden"}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
          <thead><tr>{["Employee","Bonus (BDT)","Status","Paid Date",""].map(h=><th key={h} style={{...th,fontSize:"11px"}}>{h}</th>)}</tr></thead>
          <tbody>{bp.distributions.map((d,di)=>{const emp=data.employees.find(e=>e.id===d.empId);return(
            <tr key={di}><td style={{...td,fontWeight:500}}>{emp?.name||"—"}</td><td style={{...td,fontWeight:600,color:C.purple}}>{fmtBDT(d.amount)}</td><td style={td}><span style={badge(d.status==="paid"?"green":"amber")}>{d.status}</span></td><td style={td}>{fmtDate(d.paidDate)}</td><td style={td}>{d.status==="pending"&&bp.status==="approved"&&<button style={{...btnP,padding:"3px 10px",fontSize:"11px"}} onClick={()=>markDistPaid(bp.id,d.empId)}>Mark Paid</button>}</td></tr>
          );})}
          </tbody>
        </table>
      </div>);
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"16px"}}>
      ...
      <div style={card}>
        ...
        {myPayments.length===0?<Empty msg="No payments recorded."/>:PAYMENT_TYPES.map(t=>{const tot=myPayments.filter(p=>p.type===t).reduce((s,p)=>s+p.amount,0);if(!tot)return null;return<PLRow key={t} label={t} val={fmt$(tot)} color={C.purple}/>;}).filter(Boolean)}
...
function LLCOwnerJobsView({data,currentUser}){
  ...
  return(<div style={card}>
    ...
    {myJobs.length===0?<Empty msg="No work orders yet."/>:myJobs.map(j=>{
      const vendors=j.vendors||[];const totalPaid=jobTotalPaid(j);const rcv=jobAmountReceived(j.id,data.receipts);const open=expanded===j.id;const st=arStatus(j,data.receipts);
      return(<div key={j.id} style={{border:`1px solid ${open?C.indigoBorder:C.border}`,borderRadius:"8px",marginBottom:"10px",overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px",padding:"12px 14px",cursor:"pointer",background:open?C.indigoBg:C.card}} onClick={()=>setExpanded(open?null:j.id)}>
          <div style={{flex:"0 0 88px",fontSize:"12px",color:C.textMid}}>{fmtDate(j.date)}</div>
        ...
        {open&&(<div style={{borderTop:`1px solid ${C.indigoBorder}`,background:"#f8f9ff",padding:"14px 16px"}}>
          {vendors.length===0?<div style={{fontSize:"13px",color:C.textMuted}}>No vendors.</div>:vendors.map(v=>(
            <div key={v.id} style={{marginBottom:"8px",border:`1px solid ${C.border}`,borderRadius:"6px",overflow:"hidden",background:C.card}}>
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",background:C.grayBg}}>
                <span style={{fontWeight:500}}>{v.name}</span>
                <span style={{fontWeight:600,color:C.red}}>{fmt$(vendorTotalPaid(v))}</span>
              </div>
              {v.payments.map((p,i)=>(
                <div key={p.id} style={{display:"flex",gap:"12px",padding:"6px 12px",borderTop:`1px solid ${C.bg}`,fontSize:"12px"}}>
...
const ANAV=[{id:"dashboard",label:"Overview",icon:"▦"},{id:"groups",label:"Portfolio Groups",icon:"◈"},{id:"llcs",label:"LLC Entities",icon:"▤"},{id:"jobs",label:"Jobs & Revenue",icon:"◎"},{id:"receipts",label:"Client Receipts",icon:"◑"},{id:"llcpayments",label:"LLC → Ascentix Payments",icon:"◷"},{id:"expenses",label:"Expenses",icon:"▷"},{id:"payroll",label:"Payroll",icon:"◉"},{id:"capital",label:"Working Capital",icon:"◫"},{id:"reports",label:"Reports",icon:"⊞"}];
...
function AdminApp({data,onSave,showToast,currentUser,onLogout,toast}){
  const [nav,setNav]=useState("dashboard");
  const props={data,onSave,showToast,currentUser};
  const pages={dashboard:<Dashboard {...props}/>,groups:<GroupsPage {...props}/>,llcs:<LLCsPage {...props}/>,jobs:<JobsPage {...props}/>,receipts:<ClientReceiptsPage {...props}/>,llcpayments:<LLCPaymentsPage {...props} viewLlcId={null}/>,expenses:<ExpensesPage {...props}/>,payroll:<PayrollPage {...props}/>,capital:<CapitalPage {...props}/>,reports:<ReportsPage {...props}/>};
  return(<div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"system-ui,-apple-system,sans-serif",fontSize:"14px",lineHeight:1.5}}>
    <nav style={{width:"210px",borderRight:`1px solid ${C.border}`,background:C.card,display:"flex",flexDirection:"column",flexShrink:0,padding:"14px 10px"}}>
      ...
      {ANAV.slice(3).map(n=><NavBtn key={n.id} n={n} nav={nav} setNav={setNav}/>)}
...
export default function App(){
  ...
  return currentUser.role==="admin"?<AdminApp {...props}/>:<LLCOwnerApp {...props}/>;
}
